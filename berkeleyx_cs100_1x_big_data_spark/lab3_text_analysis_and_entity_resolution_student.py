...
def countTokens(vendorRDD):
    ...
    """
    return vendorRDD.map(lambda (k,v):len(v)).reduce(lambda a,b :a+b)
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.takeOrdered(1,lambda (k,v): -len(v))
...
def idfs(corpus):
    ...
    tokenCountPairTuple = uniqueTokens.flatMap(lambda x: [(t,1) for t in x])   
